...
from crossworld.envs.asset_path_utils import full_v2_path_for
from crossworld.envs.mujoco.jaco.jaco_env import RenderMode, JacoEnv
from crossworld.envs.mujoco.utils.reward_utils import tolerance
from crossworld.types import InitConfigDict, ObservationDict


class JacoNutAssemblyEnvV2(JacoEnv):
    ...
    def model_name(self) -> str:
        return full_v2_path_for("jaco/jaco_assembly_peg.xml")

    @JacoEnv._Decorators.assert_task_is_set
    def evaluate_state(
    ...
    def compute_reward(
        ...
        threshold = JacoNutAssemblyEnvV2.WRENCH_HANDLE_LENGTH / 2.0
        if abs(wrench[0] - hand[0]) < threshold:
            wrench_threshed[0] = hand[0]

        reward_quat = JacoNutAssemblyEnvV2._reward_quat(obs)
        ...
        reward_in_place, success = JacoNutAssemblyEnvV2._reward_pos(
