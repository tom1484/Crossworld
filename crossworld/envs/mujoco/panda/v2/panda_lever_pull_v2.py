...
class PandaLeverPullEnvV2(PandaEnv):
    """PandaLeverPullEnv.
    ...
    def model_name(self) -> str:
        return full_v2_path_for("panda/panda_lever_pull.xml")
    ...
    def compute_reward(
        ...
        # De-emphasize y error so that we get Panda's shoulder underneath the
        # lever prior to bumping on against
        scale = np.array([4.0, 1.0, 4.0])
        # Offset so that we get the Panda's shoulder underneath the lever,
