...
from crossworld.envs.mujoco.panda.panda_env import RenderMode, PandaEnv
...
class PandaNutAssemblyEnvV2(PandaEnv):
    ...
    def model_name(self) -> str:
        return full_v2_path_for("panda/panda_assembly_peg.xml")
    ...
    def compute_reward(
        ...
        threshold = PandaNutAssemblyEnvV2.WRENCH_HANDLE_LENGTH / 2.0
        if abs(wrench[0] - hand[0]) < threshold:
            wrench_threshed[0] = hand[0]

        reward_quat = PandaNutAssemblyEnvV2._reward_quat(obs)
        ...
        reward_in_place, success = PandaNutAssemblyEnvV2._reward_pos(
